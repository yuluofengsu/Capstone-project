          ...
          <div class="book-infos-view">
            <div class="book-infos">
              ...
              <div class="book-info-box">
                ...
                <div class="translators flex-view" style="">
                  <span>出版社：</span>
                  <span class="name">{{ detailData.press }}</span>
                </div>

                <div class="translators flex-view" style="">
                  <span>出版年：</span>
                  <span class="name">{{ detailData.pub_date }}</span>
                </div>

                <div class="translators flex-view" style="">
                  <span>页数：</span>
                  <span class="name">{{ detailData.page_count }}</span>
                </div>

                <div class="translators flex-view" style="">
                  <span>定价：</span>
                  <span class="name">{{ detailData.price }}</span>
                </div>

                <div class="translators flex-view" style="">
                  <span>评分：</span>
                  <span class="name">{{ detailData.score }}</span>
                </div>


                <div class="translators flex-view" style="">
                  <span>装帧：</span>
                  <span class="name">{{ detailData.layout }}</span>
                </div>


                <div class="translators flex-view" style="">
                  <span>ISBN：</span>
                  <span class="name">{{ detailData.isbn }}</span>
                </div>
    ...
    getBookDetail() {
      detailApi({id: this.bookId}).then(res => {
        ...
        if (this.detailData.cover) {
          ...
          const coverString = String(this.detailData.cover); // 确保它是字符串
          ...
          if (decodedCover.includes('http') || decodedCover.includes('https')) {
            ...
            if (decodedCover.startsWith('/upload/')) {
              this.detailData.cover = decodedCover.slice(8); // 去掉 '/upload/' 的8个字符
              // console.log("外部链接，去掉/upload/: ", this.detailData.cover);
            } else {
              this.detailData.cover = decodedCover; // 直接使用原始链接
              // console.log("外部链接，保持不变:", this.detailData.cover);
            }
          } else {
            // 如果是本地路径，拼接基础 URL 和上传路径
            this.detailData.cover = this.$BASE_URL + this.detailData.cover // 拼接本地路径
          }
        }
      }).catch(err => {
